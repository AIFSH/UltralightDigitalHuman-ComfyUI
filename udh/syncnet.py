...
def train(save_dir, dataset_dir, mode,patient,batch_size):
    ...
    with tqdm(total=40, desc='training syncnet') as p:
        for epoch in range(40):
            ...
            if skip_step == patient:
                break
            p.set_postfix(**{'epoch':epoch+1,'loss (batch)': loss_info})
            p.update(1)
        
        # print(epoch, loss.item())
        #torch.save(model.state_dict(), os.path.join(save_dir, str(epoch)+'.pth'))
    print(f"min loss is {min_loss} at epoch {min_loss_step}")
    torch.save(model_state, os.path.join(save_dir, 'min_loss.pth'))   
...
if __name__ == "__main__":
    ...
    train(opt.save_dir, opt.dataset_dir, opt.asr,opt.patient,opt.batch_size)
